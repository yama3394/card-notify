...
- **全取引に連番 ID**（`#101` 形式。WebUI・CSV に表示）。現金の登録は LINE から**当日中なら取り消せる**
...
| `今日` | 今日登録済みの支出一覧と合計を返信（現金には ID 付き） |
| `今週` | 今週の日別一覧を返信 |
| `今月のまとめ` / `月次レポート` / `まとめ` | 月次レポートを即時再送 |
| `取消`（`取消し` / `取り消し` / `とりけし` も可） | 直前の現金登録を取り消す（1件だけ。続けて送っても2件目は消えない） |
| `#101 取消` / `取消 #101` | ID を指定して現金登録を取り消す |

金額は先頭トークンが数字（カンマ区切り可）である必要がある。認識できない形式は
使い方の案内を返信する。

登録の返信には `ID: #101` のように連番 ID が付く。取り消せるのは**今日送信した現金の
登録だけ**で、「今日」は支出日ではなく送信日で判定する（`500 コンビニ 昨日` を今日
送ったなら今日中は取り消せ、23:59 に送ったものは日付が変わると取り消せない）。
カードの取引と前日以前の登録は WebUI から削除する。v1.2.0 より前に登録した取引にも
ID は付くが、送信時刻の記録が無いため LINE からは取り消せない。すでに日次通知で
報告済みの日の取引を取り消した場合は、翌朝の日次通知の「取消反映」欄で知らせる。
